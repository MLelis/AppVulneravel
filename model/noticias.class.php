...
class Noticias {
 ...
 function listarTodasNoticias() {	
 	 	$sql = "SELECT
			...
			if($this->categoria){
				$sql .= " AND id_categoria = '".$this->categoria."'";
			}

			if($this->ordenarPor){
				$sql .= " ORDER BY ".$this->ordenarPor."";
			}
  ...
  function retornarNoticia(){
  	...
  	if($this->idnoticia){
		$sql .= " AND n.id_noticia = '".$this->idnoticia."'";
	}

  	if($this->destaque){
		$sql .= " AND n.destaque = '".$this->destaque."'";
	}
  ...
  function modificarNoticia(){
 	...
 	if($this->post['descricaoNoticia']){
 		$sql .= " descricao = '".utf8_decode($this->post['descricaoNoticia'])."', ";
 	}
  ...
  function listarTodosParceiros(){
      ...
      while($linha = mysqli_fetch_array($query, MYSQLI_ASSOC)){
        $this->parceiros[$linha['id_parceiro']] = $linha;
      }
